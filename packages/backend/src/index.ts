...
app.get("/api/health", async (req, res) => {
  try {
    ...
    res.json({ status: "ok", dbTime: result.rows[0], neonAuth: "enabled" });
  } catch (error: any) {
    res.status(500).json({ status: "error", message: error.message });
  }
});

// Default Initial State for Agro ERP
export const initialERPState = {
  suppliers: [
...
/**
 * Persists the individual, relational normalized tables to correspond with the state.
 * This guarantees the database's relational models remain in sync with state actions.
 */
export async function syncNormalizedTables(state: any) {
  try {
    console.log("Synchronizing relational normalized tables...");
    
    // 1. DELETE PHASE: Delete from children first, then parents, to satisfy foreign key constraints
    if (state.inventory && Array.isArray(state.inventory)) {
      await db.delete(inventory);
    }
    if (state.batches && Array.isArray(state.batches)) {
      await db.delete(batches);
    }
    if (state.warehouses && Array.isArray(state.warehouses)) {
    ...
    // 2. INSERT PHASE: Insert parents first, then children
    // Suppliers (Parent)
    if (state.suppliers && Array.isArray(state.suppliers)) {
      for (const item of state.suppliers) {
    ...
    // Warehouses (Parent of Inventory and Batches)
    if (state.warehouses && Array.isArray(state.warehouses)) {
      for (const item of state.warehouses) {
    ...
    if (state.employees && Array.isArray(state.employees)) {
      for (const item of state.employees) {
        await db.insert(employees).values({
          ...
          attendanceRate: Number(item.attendanceRate) || 100,
    ...
    console.log("Relational normalized database successfully synchronized.");
  } catch (error) {
    console.error("Failed to sync relational database tables:", error);
    throw error; // Throw so that calling endpoint knows it failed
  }
}

// 1. GET: Fetch current ERP State
app.get("/api/erp/state", async (req, res) => {
  try {
    const rows = await db.select().from(agroErpState).where(eq(agroErpState.id, "current_state"));
    if (rows.length > 0) {
      res.json(rows[0].stateJson);
    } else {
      // Lazy seed on-the-fly if missing
      await db.insert(agroErpState).values({
        id: "current_state",
        stateJson: initialERPState,
        updatedAt: new Date()
      });
      await syncNormalizedTables(initialERPState);
      res.json(initialERPState);
    }
  } catch (error: any) {
    console.error("Error retrieving ERP State:", error);
    res.status(500).json({ error: error.message });
  }
});

// 2. POST: Save updated ERP State (validated with Zod)
app.post("/api/erp/state", async (req, res) => {
  try {
    ...
    // Validate using Zod schema from shared-utils
    const validation = ERPStateSchema.safeParse(state);
    if (!validation.success) {
      console.warn("Zod validation failure for ERP State:", validation.error.format());
      return res.status(400).json({
        error: "Invalid ERP State structure",
    ...
    // Upsert into state jsonb table
    await db.insert(agroErpState)
      .values({
        id: "current_state",
        stateJson: validatedState,
        updatedAt: new Date()
      })
      .onConflictDoUpdate({
    ...
    // Sync individual tables asynchronously
    await syncNormalizedTables(validatedState);

    res.json({ success: true });
  } catch (error: any) {
    console.error("Error writing ERP State:", error);
    res.status(500).json({ error: error.message });
  }
});
...
app.post("/api/erp/reset", async (req, res) => {
  try {
    console.log("Resetting database state to defaults...");
